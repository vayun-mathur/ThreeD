...
float4 psmain(PS_INPUT input) : SV_TARGET
{
	...
	for (int i = 0; i < m_dlight_count; i++) {
		light += tex_color * calculateDirectional(material, dlight[i], input.world_pos, input.normal);
	}
	for (int i = 0; i < m_plight_count; i++) {
		light += tex_color * calculatePoint(material, plight[i], input.world_pos, input.normal);
	}
